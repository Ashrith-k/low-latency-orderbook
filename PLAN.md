...
1. `.gitignore`, MIT `LICENSE`, README stub with one-paragraph pitch. [chore: repo scaffold]
2. Root `CMakeLists.txt` (C++20, warnings-as-errors: `-Wall -Wextra -Wpedantic -Werror`). [build: cmake skeleton]
3. `CMakePresets.json`: debug / asan / tsan / release presets. [build: presets]
4. `.clang-format` + `scripts/format.sh`. [chore: formatting]
5. `.clang-tidy` config. [chore: clang-tidy]
6. FetchContent GoogleTest + one passing dummy test. [test: wire gtest]
7. FetchContent Google Benchmark + one dummy bench. [bench: wire gbench]
8. `.github/workflows/ci.yml`: build+test matrix {gcc,clang} × {debug,release}. [ci: build+test]
9. Add asan job + format-check job to CI. [ci: sanitizers + format gate]
10. `include/lob/types.h`: Side, OrderType, Command, Event PODs + static_asserts. [feat: core types]
11. `NaiveBook` (std::map + std::list): add/cancel. [feat: naive book add/cancel]
12. `NaiveBook` matching (limit/market/IOC, partial fills). [feat: naive matching]
13. 15+ unit tests on NaiveBook — this is your executable spec of matching semantics. [test: matching semantics]
